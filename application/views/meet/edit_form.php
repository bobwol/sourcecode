...
<?php 
	...
	$venue				= set_value('venue', isset($meetInfo->venue) ? $meetInfo->venue : '');
	$address1			= set_value('address1', isset($meetInfo->address1) ? $meetInfo->address1 : '');
	$address2			= set_value('address2', isset($meetInfo->address2) ? $meetInfo->address2 : '');
	$city					= set_value('city', isset($meetInfo->city) ? $meetInfo->city : '');
	$state				= set_value('state', isset($meetInfo->state) ? $meetInfo->state : '');
	$zip					= set_value('zip', isset($meetInfo->zip) ? $meetInfo->zip : '');
	$contactName	= set_value('contactName', isset($meetInfo->contactName) ? $meetInfo->contactName : '');
	$contactEmail	= set_value('contactEmail', isset($meetInfo->contactEmail) ? $meetInfo->contactEmail : '');
	$contactPhone	= set_value('contactPhone', isset($meetInfo->contactPhone) ? $meetInfo->contactPhone : '');
	...
	$maxDays 			= set_value('maxDays', isset($maxDays) ? $maxDays : 4);
	$maxDivs 			= set_value('maxDivs', isset($maxDivs) ? $maxDivs : 12);
	...
	if($meetID != 0) {	
		...
		$pageTitle = 'Edit Meet Information';
		$addlMenuOptions = $addlMenuOptions.'<li><a href="JavaScript:void(0);" onclick="$(\'#delLink\').fadeIn(2000);" >Delete</a></li>';
		$addlMenuOptions = $addlMenuOptions.'<li><a href="JavaScript:void(0);" onclick="performAction(\'generate\');"  >Generate Schedule</a></li>';
		if(isset($numberOfEvents))
		{
			if($numberOfEvents)
			{
				$addlMenuOptions = $addlMenuOptions.'<li><a href="JavaScript:void(0);" onclick="performAction(\'adjust\');"  >Adjust Schedule</a></li>';
			}
		}
		$addlMenuOptions = $addlMenuOptions.'<li><a href="JavaScript:void(0);" onclick="performAction(\'assign\');" >Assign Athletes</a></li>';
		$addlMenuOptions = $addlMenuOptions.'<li><a href="/meet/report/'.$meetID.'">View Report</a></li>';
	}
		...
		switch(selectedAction.toUpperCase()) {
			case 'SAVE':
				if((document.getElementById('meetType').value == "dual") && (parseInt(document.getElementById('numOfDays').value) > 1)){
					...
					document.getElementById('frmMeetInfo').action = '/meet/edit';
					document.getElementById('frmMeetInfo').submit();
				}
				break;
			case 'GENERATE':
				document.getElementById('frmMeetInfo').action = '/meet/generate_event_sched';
				document.getElementById('frmMeetInfo').submit();
				break;
			case 'ADJUST':
				document.getElementById('frmMeetInfo').action = '/meet/adjust_event_sched';
				document.getElementById('frmMeetInfo').submit();
				break;
			case 'ASSIGN':
				document.getElementById('frmMeetInfo').action = '/meet/assign_athletes';
				document.getElementById('frmMeetInfo').submit();
				break;
			case 'DELETE':
				document.getElementById('frmMeetInfo').action = '/meet/delete';
				document.getElementById('frmMeetInfo').submit();
				break;
		}
	}
...
<div class="topRegion">
	...
	<div id="delLink" class="confirm hide" style="margin-right: 434px;">Are you sure?&nbsp;
...
<?php echo form_open($this->uri->uri_string(), $frmAttributes); ?>
	...
	<input type="hidden" name="maxDays" id="maxDays" size="11" value="<?php echo $maxDays; ?>" >
	<input type="hidden" name="maxDivs" id="maxDivs" size="11" value="<?php echo $maxDivs; ?>" >
